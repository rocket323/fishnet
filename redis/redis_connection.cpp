...
RedisConnection::RedisConnection(EventLoop *event_loop, redisAsyncContext *context,
...
int RedisConnection::Do(const RedisReplyCallback &cb, int argc, const char **argv,
...
RedisConnectionPtr RedisConnection::Connect(EventLoop *event_loop, const InetAddr &addr) {
    ...
    conn =
        std::make_shared<RedisConnection>(event_loop, context, sockets::GetLocalAddr(sockfd), addr);
